...
par1mat=[];
%% 190228 HV 
load('190530_HV_keima_PBG_02-Apr-2024\190530_HV_keima_PBGnofdr.mat','par1TOT','LLRTOT','time','dirname');
par1mat=[par1mat; par1TOT];
LLRmat=[LLRmat;LLRTOT'];
n1=numel(LLRTOT);
dirname1=dirname;
%%
load('190530_HVP_keima_PBG_02-Apr-2024\190530_HVP_keima_PBGnofdr.mat','par1TOT','LLRTOT','time','dirname');
par1mat=[par1mat; par1TOT];
LLRmat=[LLRmat; LLRTOT'];
dirname2=dirname;
...
histogram(LLRM,grid), title(' data')
subplot(2,1,2)
...
for i = 1:length(binnings)
    Thresh = binnings(i);
    q1(i) = piGUESS1*(sum(LLRS>=Thresh)/length(LLRS))/(sum(LLRM>=Thresh)/length(LLRM));
end
...
try
    ...
    disp('No oscillators found')
end
%%
save globalFDR_190530HV_HVP_Keima.mat
