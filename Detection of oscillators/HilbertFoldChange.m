...
for i=1:numel(dfit)
    ...
    subplot(2,1,1)
    for j=1:numel(fc1)
        text(pair(j,1),1.1*fittedsig(pair(j,1)),[num2str(round(fc1(j)*100)/100),'x']);
    end
    ylim=get(gca,'YLim');
    set(gca,'YLim',[ylim(1),1.1*ylim(2)]);
    %subplot(2,1,1),legend('Trend','Model fit','Amplitude max','Amplitude min','Orientation','Horizontal','Location','NorthOutside');
    subplot(2,1,1),legend('Trend','Model fit','Amplitude');%,'Orientation','Horizontal','Location','NorthOutside');
